"""GPU Service — TRELLIS.2 reconstruction only. Model loaded at startup."""
...
app = FastAPI(title="TRELLIS.2 Service", version="2.0.0")

WORK_DIR = Path(tempfile.mkdtemp(prefix="trellis_"))
...
async def startup_load_model():
    global _pipeline
    logger.info("Loading TRELLIS.2 model at startup...")
    from trellis_wrapper import load_trellis_model
    _pipeline = load_trellis_model()
    logger.info("TRELLIS.2 ready.")
...
if __name__ == "__main__":
    uvicorn.run("server_trellis:app", host="0.0.0.0", port=8877, reload=False)
